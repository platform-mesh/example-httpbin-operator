...
A kind cluster with platform-mesh configured and example-httpbin-operator deployed via ocm.
...
Install the api-syncagent with flux:
    ...
    kubectl apply -f hack/platform-mesh/kcp-api-syncagent/api-syncagent.yaml
...
resources that can be ordered in the consumer cluster:
    ...
    kubectl apply -f hack/platform-mesh/kcp-api-syncagent/rbac.yaml
...
synchronize between the provider and consumer clusters.
    ...
    kubectl apply -f ./hack/platform-mesh/kcp-api-syncagent/publishedresources.yaml
